...
const CONFIG = {
  ...
  // --- マンデルボックスのパラメータ ---
  // 参考サイト(hausdorff-dimension.netlify.app の mandelbox.frag)の
  // マンデルボックスDEパラメータをそのまま移植(README「パラメータの
  // 選定」参照)。SCALEが負なのが特徴で、反復ごとに折り畳みの向きが
  // 反転する独特の見た目になる。
  SCALE: -2.18,
  MINR2: 0.60,
  FIXEDR2: 2.65,
  FOLD: 1.14,
  ITER: 16,
  // Node.js での実測値(対角方向で最大~3.95、軸方向は~2.28。安全マージンを
  // 見て4.2に設定。README「パラメータの選定」参照)。
  BOUND_RADIUS: 4.2,
  ...
  // uMaxDist(レイの探索打ち切り距離) = 現在のカメラ距離 * FAR_MULT。
  // 「表面が四角い点の集まりに薄れて見える」不具合の本当の原因は精度でも
  // ステップ数でもなく、この探索半径が狭すぎたことだった——画面端寄りの
  // レイほどp0の方向から角度がズレるため、旧FAR_MULT=6では実際には
  // すぐ先に存在する構造まで届く前に探索を打ち切っていた(README
  // 「探索半径が狭すぎる問題」参照)。
  //
  // ただしこの広い探索半径が必要になるのは深く潜った時だけで、overview
  // (dist=OVERVIEW_DIST)付近では FAR_MULT=6 でもヒット率はほぼ100%に
  // 達していた(Node.js上でdist=13〜5e-5・FAR_MULT=6/15/40を総当たりで
  // 実測、README「探索半径を適応的に変える」参照)。そこで固定値ではなく
  // 現在のズーム進捗(zoomT: overviewで0、DIST_MINで1)に応じて線形補間
  // する。ズームが浅い間は探索を短く切り上げてステップを節約できる。
  FAR_MULT_MIN: 6.0, // overview(zoomT=0)でのFAR_MULT
  FAR_MULT_MAX: 40.0, // DIST_MIN(zoomT=1)でのFAR_MULT
  // フォグの奥行き感は探索半径(FAR_MULT)とは別のスケールで決めたい
  // ため、独立した係数として持つ(uFogMaxDist = dist * FOG_MULT)。
  // 探索半径を大きく伸ばした後もフォグの掛かり方が変わらないよう、
  // 旧FAR_MULTの値をそのまま引き継いでいる。
  FOG_MULT: 6.0,
  FOG_K: 1.5,

  // --- 配色・ライティング(参考: https://hausdorff-dimension.netlify.app/mandelbox.html の mandelbox.frag) ---
  // 参考サイトの incCenter(軸±2)・incRadius(2.0)をそのまま採用。
  // OVERVIEW_DIST(11)が参考サイト自身のカメラ距離とちょうど一致した
  // ため、スケール変換が不要になった。
  GLOW_RADIUS: 2.0,
  // ガンマ補正前の線形色に掛ける明るさ倍率(色相・彩度は不変)。参考
  // サイトは常にオブジェクトへ寄って複雑な構造を画面いっぱいに映すため
  // growの寄与が常に大きく「燃えている」ように見えるが、overviewでは
  // 遠くから単純な形状を見るぶんレイマーチのステップ消費が少なく、
  // growがほとんど乗らずに素のambient+diffuseの落ち着いた色になって
  // しまう。BRIGHTNESSを底上げしてこの土台の明るさを持ち上げている。
  BRIGHTNESS: 1.5,
  ...
  // さらにoverviewのように遠くから単純な形状を見る場面ではステップ消費
  // 自体が少ないため、閾値を大きく緩めて低いiterateでも強くかかるように
  // している(README「見た目が単調になる問題」参照)。
  GROW_ITER_LO: 0.0,
  GROW_ITER_HI: 0.12,

  // --- ズームサイクル ---
  // 全体を見渡す距離。旧パラメータでは BOUND_RADIUS(5.0) との比 2.6 で
  // 13.0 に設定していたので、同じ比を新しい BOUND_RADIUS(4.2) に当てはめた
  // (≒10.9、キリよく11.0に。偶然にも参考サイト自身のカメラ距離である
  // 11 と一致する——README参照)。
  OVERVIEW_DIST: 11.0,
  // FAR_MULT修正後、Node.js上で15個のpickTarget()先・169本のレイ格子で
  // 実測したヒット率は dist=1e-4で86%, 5e-5で82%, 3e-5で77%, 1e-5で57%
  // (README「探索半径が狭すぎる問題」参照)。ステップ消費数がMAX_STEPSに
  // 近づき始める深さでもあるため、確実に密な描画を保てる範囲として
  // 5e-5を採用した。
  DIST_MIN: 5e-5,
...
// フォールバック(乱数探索が万一全滅した場合の既知の有効点。参考サイトの
// DEパラメータに変更した際、Node.js上でpickTarget()を30回試行して
// 0/30フォールバックを確認した上で、その中の1点を採用した)。
...
uniform vec3 uR;
uniform vec3 uU;
uniform float uMaxDist; // レイの探索打ち切り距離(= dist * FAR_MULT)
uniform float uFogMaxDist; // フォグの基準距離(= dist * FOG_MULT)。uMaxDistとは独立
...
const uLoc = {
  ...
  fogMaxDist: gl.getUniformLocation(program, "uFogMaxDist"),
};
...
// 設計(README「3段のサイクル」参照)に合わせ、log10距離で線形補間する。
// HUDの表示だけでなく、FAR_MULTの適応的な補間にも使う。
function zoomProgress(dist) {
  const zoomExp = Math.log10(Math.max(CONFIG.OVERVIEW_DIST / dist, 1));
...
function frame(now) {
  ...
  try {
    ...
    // FAR_MULTはoverview(zoomT=0)からDIST_MIN(zoomT=1)にかけて線形補間
    // する。浅いズームでは広い探索半径は不要なので、無駄なステップを
    // 節約できる(README「探索半径を適応的に変える」参照)。
    const farMult = CONFIG.FAR_MULT_MIN + (CONFIG.FAR_MULT_MAX - CONFIG.FAR_MULT_MIN) * zoomProgress(dist);
    const maxDist = dist * farMult;
    const fogMaxDist = dist * CONFIG.FOG_MULT;
    ...
    gl.uniform1f(uLoc.fogMaxDist, fogMaxDist);
