...
// 参考実装(prototype-mandelbox2)からの移植。ズームイン(dive)は行わず、
// 固定距離(OVERVIEW_DIST、index.js側)から見回す・時々別の地点へフェード
// する「見せ場」用途に絞ったため、参考実装のズームHUD/ホイール操作等は
// 持たない。DE・ライティングのロジックは参考実装と同一(移植時の改変なし)。
// Works一覧のカバーフロー同様、このブロックは共有WebGL2レンダラの1ブロック
// として動く。他ブロックとcontextを共有するため、レイマーチは軽量化のため
// 縮小した専用オフスクリーンFBOへ描き、index.js側の2パス目でアップスケール
// する(rain-windowブロックと同じ手法)。このシェーダー自体はFBOにフル
// フラグメントを描くだけなので、gl_FragCoordはFBOローカル座標のまま
// (canvas全体のoriginを引く必要はない)。
...
const int Iteration = 170; // 参考実装は280。ズームインしないため到達距離が浅く、この値でも十分収束する
...
// GLSL ES 300のビルトインinverse()(行列専用)と名前が衝突するため
// quatInverseに変更(ロジックは参考実装と同じ)。
...
vec4 turn(vec4 v,vec4 rot){
  return times(times(rot,v),quatInverse(rot));
}
...
void main(void){
  ...
  fragColor = vec4(ray.fragColor,1.0);
}
