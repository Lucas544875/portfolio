...
// 手持ちカメラのような微小なランダム揺れ。周波数の異なる3つのサイン波を
// 位相をずらして重ねるだけの簡易な疑似ノイズ。戻り値はおおよそ[-1,1]。
...
// 手持ちカメラのような微小なランダム揺れ。位置には影響させない。
const SHAKE_AMPLITUDE = 0.01; // rad
...
function frame(now){
  ...
  const shakeQuat = normalizeQuat(
    Quatarnion.rotation(shakeYaw, cam.U[0], cam.U[1], cam.U[2])
      .times(Quatarnion.rotation(shakePitch, cam.R[0], cam.R[1], cam.R[2]))
  );
